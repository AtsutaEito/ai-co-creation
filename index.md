...
| 脳力（縦軸） | 創作（実装）ループにおける役割 | 進化（改善）ループにおける役割 | 理解（探求）ループにおける役割 |
| :--- | :--- | :--- | :--- |
| **アーキテクチャ思考**<br>（思考の構造化脳力） | 脳内の曖昧なイメージを、AIが処理できる論理的な情報（TOML、DOT等の構造化データ）に変換しインプットする。 | 対話で生じた「ズレ（不整合）」を特定し、仕組み（ドキュメント）のどのルールの変更によってバグを塞ぐべきか、構造をKaizen（再設計）する。 | AIの演算特性の「偏り・バグ」を客観的な挙動パラメータ（性質）として抽出し、人間側の前提モデルのトポロジー（論理設計）を更新する。 |
| **ステートマネジメント**<br>（知性の動的管理脳力） | トークン消費に伴う文脈の摩耗（アテンション飽和）を検知し、対話のフェーズ（現在地）を管理・操舵する。 | AIの「サボり」や「おべっか」による脱線を検知した瞬間、対話を一時停止（ブレイク）し、セッションの状態を再同期する。 | 普遍的な性質を探求するため、メイン対話からあえてセッションを「分岐・隔離」し、思索用の独立空間（ステート）を能動的に管理する。 |
| **コンセプト・ズーミング**<br>（概念の垂直往復脳力） | AIの出す滑らかな一般論（言葉遊び）を拒絶し、自らの具体的な事実（経験）へと引きずり下ろし、再び高次の概念（一次情報）へと昇華する。 | 仕組み（ルール）のKaizenにおいて、抽象的な精神論を書き込むのではなく、具体のエラーに直結する「厳格な物理ルール（仕様）」に翻訳してドキュメントに固定する。 | AIの不完全な演算特性（挙動）の背後にある、トランスフォーマーの普遍的な「法則（抽象）」を論理的に発見・言語化する。 |
| **マスタリー**<br>（執念深い意志の維持脳力） | AIのおべっか（やったふり）や、自分自身の妥協に屈せず、人間にしかできない独自の「問い（火種）」を最後まで燃やし続ける。 | AIの一時しのぎのなめらかな謝罪（自己分析の捏造）に流されず、仕組み自体の根治（プロトコルの書き換え）に執念深く取り組む。 | 失敗やアテンション崩壊に屈することなく、AIの想定外の気付きの深奥にある普遍的な真実を、執念深く追求し、人間自身の限界を突破する。 |
